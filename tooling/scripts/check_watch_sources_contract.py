...
def main() -> int:
    ...
    parser.add_argument("--workspace-root", default="~/.fileman/workspaces/default")
    args = parser.parse_args()

    path = Path(args.workspace_root).expanduser() / ".fileman" / "preferences" / "watch_sources.json"
    issues: list[str] = []
    if ".fileman/preferences/" not in path.as_posix():
