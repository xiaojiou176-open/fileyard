...
def _sanitize_public_runtime_value(value: str) -> str:
    sanitized = str(value)
    replacements = [
        ("~/.fileman/workspaces/default/.fileman", "<workspace-root>/.fileman"),
        ("~/.fileman/workspaces/default/data/raw", "<workspace-root>/data/raw"),
        ("~/.fileman/workspaces/default/data/organized", "<workspace-root>/data/organized"),
        ("~/.fileman/workspaces/default/data", "<workspace-root>/data"),
        ("~/.fileman/workspaces/default", "<workspace-root>"),
...
def runtime_topology_snapshot(repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    env_defaults_subset = {
        ...
        for key in (
            "GEMINI_MODEL",
            "FILEMAN_WEB_API_HOST",
            "FILEMAN_WEB_API_PORT",
            "FILEMAN_WEBUI_HOST",
            "FILEMAN_WEBUI_PORT",
            "FILEMAN_COMPOSE_SERVICE",
            "FILEMAN_CI_IMAGE",
        )
    ...
    package_smoke = dict(pyproject.get("tool", {}).get("fileman", {}).get("package_smoke", {}))
    ...
    return {
        ...
        "cleanup_rails": [
            ...
            },
            {
                ...
                "note": "Clears workspace .fileman state; not a routine cache cleanup command.",
            },
        ],
    ...
    if docker_runtime:
        ...
        lines.extend(
            [
                ...
                "",
                f"- Canonical Docker image: `{docker_runtime.get('canonical_image', 'fileman-ci:local')}`",
...
def render_runtime_topology_summary_block(output_path: str, repo_root: Path = REPO_ROOT) -> str:
    ...
    api_port = snapshot["env_defaults"].get("FILEMAN_WEB_API_PORT", "18080")
    webui_port = snapshot["env_defaults"].get("FILEMAN_WEBUI_PORT", "5173")
    lines = [
