...
COMPOSE_SERVICE="${FILEMAN_COMPOSE_SERVICE:-fileman-ci}"
IMAGE_REF="${FILEMAN_CI_IMAGE:-}"
...
if is_inside_container; then
  exec env FILEMAN_IN_CONTAINER=1 FILEMAN_ALLOW_HOST_EXECUTION=0 "$@"
fi

if [ "${FILEMAN_IN_CONTAINER:-0}" = "1" ]; then
  echo "❌ ${LABEL}: FILEMAN_IN_CONTAINER=1 is set, but runtime is not inside a container" >&2
  echo "Do not set FILEMAN_IN_CONTAINER manually on host; use container_exec.sh directly." >&2
  exit 1
fi
...
emit_local_image_build_failure_diagnostic() {
  ...
  case "$build_output" in
    *"Temporary failure resolving"*|*"Could not resolve"*|*"could not resolve"*|*"no such host"*|*"lookup "*|*"deb.debian.org"*|*"failed to fetch"*|*"Could not connect to"*|*"Network is unreachable"*)
      ...
      echo "Next: retry on a healthy network, or prebuild/pull ${LOCAL_FALLBACK_IMAGE} (or set FILEMAN_CI_IMAGE to a reachable prebuilt image) before rerunning." >&2
      ;;
    *)
...
runtime_volume_name() {
  ...
  printf 'fileman-%s-%s' "$kind" "$suffix"
}

if is_ci_context && [ "${FILEMAN_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
  echo "❌ ${LABEL}: FILEMAN_ALLOW_HOST_EXECUTION=1 is forbidden in CI" >&2
  exit 1
fi

if ! command -v docker >/dev/null 2>&1; then
  echo "❌ ${LABEL}: docker is required when FILEMAN_ALLOW_HOST_EXECUTION!=1" >&2
  echo "Set FILEMAN_ALLOW_HOST_EXECUTION=1 only for emergency host execution." >&2
  exit 1
fi

if [ "${FILEMAN_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
  echo "⚠️ ${LABEL}: emergency host execution enabled (FILEMAN_ALLOW_HOST_EXECUTION=1)"
  exec "$@"
fi
...
CONTAINER_VENV_DIR="/root/.cache/fileman/venv/default"
CONTAINER_XDG_CACHE_HOME="/root/.cache/fileman/xdg"
CONTAINER_PIP_CACHE_DIR="/root/.cache/fileman/pip"
CONTAINER_PLAYWRIGHT_CACHE_DIR="/root/.cache/fileman/playwright"
...
venv_dir="${FILEMAN_VENV_DIR:-'"$CONTAINER_VENV_DIR"'}"
prebuilt_venv_dir="${FILEMAN_PREBUILT_VENV_DIR:-/opt/fileman-ci-venv}"
...
# dedicated volume mounted at /root/.cache/fileman/playwright. Keep the
...
hash_file="$venv_dir/.fileman_req_hash"
prev_hash=""
if [ -f "$hash_file" ]; then
...
if [ "$needs_bootstrap" = "1" ]; then
  prebuilt_hash=""
  if [ -x "$prebuilt_venv_dir/bin/python" ] && [ -f "$prebuilt_venv_dir/.fileman_req_hash" ]; then
    prebuilt_hash="$(cat "$prebuilt_venv_dir/.fileman_req_hash" 2>/dev/null || true)"
  fi
...
if [ -n "$IMAGE_REF" ]; then
  ...
  exec "${DOCKER_BIN[@]}" run --rm \
    ...
    -w /workspace \
    "${ci_passthrough_args[@]}" \
    -e FILEMAN_IN_CONTAINER=1 \
    -e FILEMAN_ALLOW_HOST_EXECUTION=0 \
    -e FILEMAN_VENV_DIR="$CONTAINER_VENV_DIR" \
    -e GEMINI_API_KEY \
    -e GEMINI_MODEL \
    -e FILEMAN_LIVE_TEST_URL \
    -e FILEMAN_ROLLBACK_HMAC_KEY \
    -e FILEMAN_TRACE_ID \
    -e FILEMAN_SESSION_ID \
    -e FILEMAN_REQUEST_ID \
    -e FILEMAN_USER_ID \
    -e FILEMAN_RUN_LIVE_TESTS \
    -e FILEMAN_ALLOW_EXTERNAL \
