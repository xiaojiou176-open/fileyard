...
REQ_HASH_FILE="$VENV_PATH/.fileman_req_hash"
...
restore_prebuilt_venv_if_available() {
  local req_hash="$1"
  local prebuilt_dir="${FILEMAN_PREBUILT_VENV_DIR:-}"
  local prebuilt_hash=""

  if [ -z "$prebuilt_dir" ] || [ ! -x "$prebuilt_dir/bin/python" ] || [ ! -f "$prebuilt_dir/.fileman_req_hash" ]; then
    return 1
  fi

  prebuilt_hash="$(cat "$prebuilt_dir/.fileman_req_hash" 2>/dev/null || true)"
