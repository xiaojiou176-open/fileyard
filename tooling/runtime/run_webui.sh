...
HOST="${FILEMAN_WEBUI_HOST:-127.0.0.1}"
PORT="${FILEMAN_WEBUI_PORT:-5173}"
SKIP_INSTALL=0
...
if [ "${FILEMAN_IN_CONTAINER:-0}" != "1" ]; then
  ...
  env FILEMAN_COMPOSE_SERVICE=fileman-webui bash "$ROOT/scripts/container_exec.sh" --label run-webui -- bash tooling/runtime/run_webui.sh "$@"
  run_rc=$?
