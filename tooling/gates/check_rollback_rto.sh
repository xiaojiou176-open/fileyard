...
BUDGET_MS="${FILEMAN_ROLLBACK_RTO_BUDGET_MS:-3000}"
...
"$VENV/bin/python" - "$REPO_ROOT" "$VENV/bin/python" "$REPO_ROOT/apps/cli/fileman.py" "$BUDGET_MS" "$SUMMARY_PATH" <<'PY'
import json
import os
import subprocess
import sys
...
with tempfile.TemporaryDirectory(prefix="rollback-rto-", dir=str(runtime_temp_root)) as tmp_dir:
    ...
    old_key = os.environ.get("FILEMAN_ROLLBACK_HMAC_KEY")
    ...
    try:
        ...
        if old_key is None:
            ...
            os.environ["FILEMAN_ROLLBACK_HMAC_KEY"] = old_key
...
summary = {
    ...
    "command": "fileman.py rollback --strict-integrity --dry-run --allowed-root <tmp>",
}
