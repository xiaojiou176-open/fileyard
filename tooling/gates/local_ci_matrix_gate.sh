...
ensure_matrix_image() {
  local py_ver="$1"
  local image_var_name="MOVI_CI_IMAGE_PY${py_ver/./}"
  ...
  local_tag="movi-ci:local-py${py_ver/./}"
...
matrix_venv_volume() {
  local py_ver="$1"
  printf 'movi-matrix-venv-py%s' "${py_ver/./}"
}
...
run_one() {
  ...
  if docker run --rm \
    ...
    bash -lc '
      set -euo pipefail
      # Matrix contract note: export MOVI_VENV_DIR="/opt/movi-ci-venv"
      # The py3.10/py3.12 local matrix consumes the image-baked, hash-locked runtime
      # directly from /opt/movi-ci-venv. Do not shadow it with a mounted repo/runtime venv.
      ...
      /opt/movi-ci-venv/bin/pytest -q -o addopts= --maxfail=1 --strict-config --strict-markers tests/unit
