...
if [ "${MOVI_IN_CONTAINER:-0}" != "1" ]; then
  ...
  if [ "${LINT_FRONTEND_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
    ...
    env -u MOVI_ALLOW_HOST_EXECUTION MOVI_COMPOSE_SERVICE=movi-web-api bash "$ROOT/scripts/container_exec.sh" --label lint-frontend -- bash tooling/gates/lint_frontend.sh "$@"
    rc=$?
    cleanup_host_webui_mountpoint
    exit "$rc"
  fi
