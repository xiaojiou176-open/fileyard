...
run_mutation_canary_in_repo_snapshot() {
  ...
  # walking it.
  snapshot_root="$(mktemp -d "/tmp/movi-mutation-snapshot.XXXXXX")"
  (
    cd "$REPO_ROOT"
    tar \
      --exclude=.git \
...
if [ "${MOVI_IN_CONTAINER:-0}" != "1" ] && [ "${MOVI_ALLOW_HOST_EXECUTION:-0}" != "1" ] && [ "${QUALITY_GATE_CONTAINER_BOOTSTRAPPED:-0}" != "1" ]; then
  run_canonical_container_wrapper "$@"
  exit $?
fi
...
run_step_with_heartbeat \
  pytest \
  run_pytest_with_isolated_tmp \
  env -u PRE_COMMIT_FROM_REF -u PRE_COMMIT_TO_REF MOVI_RUN_LIVE_TESTS=0 \
...
if [ "${MOVI_RUN_LIVE_TESTS:-0}" = "1" ]; then
  resolve_var_prefer_dotenv GEMINI_API_KEY ""
  if [ -z "${GEMINI_API_KEY:-}" ]; then
    echo "❌ quality_gate: MOVI_RUN_LIVE_TESTS=1 but GEMINI_API_KEY is missing" >&2
    exit 1
  fi
  resolve_var_prefer_env_then_runtime_env GEMINI_MODEL "gemini-3-flash-preview"
  : "${MOVI_LIVE_TEST_URL:=https://docs.github.com/en}"
  resolve_var_prefer_env_then_runtime_env MOVI_LIVE_TEST_URL "https://docs.github.com/en"
