...
MODE="${1:-${MOVI_PRE_PUSH_MODE:-standard}}"
...
if [ "${MOVI_IN_CONTAINER:-0}" != "1" ] && [ "${MOVI_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
  echo "⚠️ pre_push_gate: emergency host execution enabled (MOVI_ALLOW_HOST_EXECUTION=1)." >&2
fi
...
RANGE_FLAGS=()
if [ "${MOVI_REQUIRE_NON_EMPTY_RANGE:-0}" = "1" ]; then
...
step() {
  ...
  case "$name" in
    ...
    feature-state-layout)
      echo "Hint: durable workbench state must stay under <workspace-root>/.movi, not repo root or repo runtime cache." >&2
      ;;
