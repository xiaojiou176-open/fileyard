...
def setup_logger(level: str, json_mode: bool) -> logging.Logger:
    logger = logging.getLogger("fileman")
    ...
    run_events_path = str(os.getenv("FILEMAN_RUN_EVENTS_PATH", "")).strip()
    if run_events_path:
...
def _enrich_fields(
    ...
    *,
    ...
    env_trace_id = _env_default("FILEMAN_TRACE_ID")
    env_request_id = _env_default("FILEMAN_REQUEST_ID")
    env_session_id = _env_default("FILEMAN_SESSION_ID")
    env_user_id = _env_default("FILEMAN_USER_ID")
    env_workspace_root = _env_default("FILEMAN_WORKSPACE_ROOT")
    trace_id = str(
