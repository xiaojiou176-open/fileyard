...
describe('InboxPage', () => {
  ...
  it('renders watch sources and inbox scan results', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Fileman Inbox' })).toBeInTheDocument()
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Scan Fileman Inbox' }))
  ...
  it('renders localized inbox copy when the locale is switched to zh-CN', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Fileman Inbox' })).toBeInTheDocument()
    expect(screen.getByText(/你可以把 Fileman Inbox 理解成收件台/)).toBeInTheDocument()
    expect(screen.getByPlaceholderText('来源名称')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '保存来源' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '扫描 Fileman Inbox' })).toBeInTheDocument()
  })
})
