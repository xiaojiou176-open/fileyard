...
def _restore_live_env_vars():
    keys = (LIVE_FLAG, "MOVI_LIVE_TEST_URL")
...
def _live_browser_url() -> str:
    url = resolve_live_var("MOVI_LIVE_TEST_URL", DEFAULT_LIVE_TEST_URL).strip()
    if not url:
        pytest.fail("live_browser preflight failed: MOVI_LIVE_TEST_URL is required and must point to a real external site")
    parsed = urlparse(url)
    host = parsed.hostname or ""
    if parsed.scheme != "https" or not parsed.netloc:
        pytest.fail(f"live_browser preflight failed: MOVI_LIVE_TEST_URL must be an absolute https URL, got {url!r}")
    if not _host_is_real_external(host):
        pytest.fail(f"live_browser preflight failed: MOVI_LIVE_TEST_URL must target a real external host, got host={host!r}")
    return url
