...
def _workspace_input_root() -> Path:
    return Path(os.environ.get("FILEMAN_INPUT_ROOT", "~/.fileman/workspaces/default/data/raw")).expanduser()


def _workspace_output_root() -> Path:
    return Path(os.environ.get("FILEMAN_OUTPUT_ROOT", "~/.fileman/workspaces/default/data/organized")).expanduser()
...
def _new_non_live_context(browser):
    ...
    context.add_init_script(
        """
        (() => {
          window.localStorage.setItem('fileman.locale', 'en');
...
def test_webui_playwright_non_live_full_journey(e2e_cleanup_actions, tmp_path: Path):
    ...
    run_env["FILEMAN_ALLOW_HOST_EXECUTION"] = run_env.get("FILEMAN_ALLOW_HOST_EXECUTION") or "1"
    run_env["FILEMAN_IN_CONTAINER"] = run_env.get("FILEMAN_IN_CONTAINER") or "0"
    run_env["FILEMAN_ROLLBACK_HMAC_KEY"] = run_env.get("FILEMAN_ROLLBACK_HMAC_KEY") or "webui-playwright-e2e-key"
    run_env["PYTHONPATH"] = f"{repo_root}{os.pathsep}{run_env.get('PYTHONPATH', '')}".rstrip(os.pathsep)
    run_env["FILEMAN_RUN_LIVE_TESTS"] = "0"
    run_env["FILEMAN_RUN_WEBUI_E2E"] = "0"
    workspace_root = tmp_path / "workspace"
    run_env["FILEMAN_WORKSPACE_ROOT"] = str(workspace_root)
    run_env["FILEMAN_INPUT_ROOT"] = str(workspace_root / "data" / "raw")
    run_env["FILEMAN_OUTPUT_ROOT"] = str(workspace_root / "data" / "organized")
    ...
    with api_log.open("w", encoding="utf-8") as api_log_file:
        ...
        source_dir = Path(run_env["FILEMAN_INPUT_ROOT"]).expanduser() / f"e2e-nonlive-{token}"
        ...
        with sync_playwright() as p:
            ...
            _wait_until(page.get_by_role("heading", name=re.compile(r"^Fileman Review$")), timeout_s=WAIT_TIMEOUT_S)
            ...
            execute_target = str((Path(run_env["FILEMAN_OUTPUT_ROOT"]).expanduser() / f"e2e-nonlive-{token}-renamed.png").resolve())
            _http_json_request(
...
def test_webui_playwright_non_live_upload_conflicts_preview_and_report_filters(e2e_cleanup_actions, tmp_path: Path):
    ...
    with api_log.open("w", encoding="utf-8") as api_log_file:
        ...
        with sync_playwright() as p:
            ...
            assert len(rows) >= 3
            conflict_target = str((Path(run_env["FILEMAN_OUTPUT_ROOT"]).expanduser() / f"conflict-{token}.png").resolve())
            manual_target = str((Path(run_env["FILEMAN_OUTPUT_ROOT"]).expanduser() / f"manual-target-{token}.png").resolve())
...
def test_webui_playwright_non_live_jobs_cancel(e2e_cleanup_actions, tmp_path: Path):
    ...
    token = uuid.uuid4().hex[:8]
    source_dir = Path(run_env["FILEMAN_INPUT_ROOT"]) / f"cancel-source-{token}"
    source_dir.mkdir(parents=True, exist_ok=True)
