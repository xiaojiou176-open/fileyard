...
WEBUI_E2E_FLAG = "FILEMAN_RUN_WEBUI_E2E"
WAIT_TIMEOUT_S = 180
READY_TIMEOUT_S = 60
POLL_INTERVAL_S = 0.3
...
def _activate_live_coverage_guard() -> None:
    ...
    _LIVE_COVERAGE_BACKUP_PATH = Path(tempfile.gettempdir()) / f"fileman-live-coverage-{os.getpid()}.xml"
    ...
    _LIVE_COVERAGE_DATA_BACKUP_PATH = Path(tempfile.gettempdir()) / f"fileman-live-coverage-data-{os.getpid()}"
    if _LIVE_COVERAGE_DATA_HAD_BASELINE:
        ...
        _LIVE_COVERAGE_DATA_BACKUP_PATH.unlink()
    _LIVE_COVERAGE_DATA_ISOLATED_PATH = Path(tempfile.gettempdir()) / f"fileman-live-cov-isolated-{os.getpid()}"
...
def test_live_webui_env_preflight():
    ...
    if not _resolve_live_var(WEBUI_E2E_FLAG, "0") == "1":
        pytest.skip(f"set {WEBUI_E2E_FLAG}=1 to run Fileman WebUI live e2e")
...
def test_live_webui_analyze_manifest_apply_rollback(live_cleanup_actions, tmp_path: Path):
    if not _is_live_enabled():
        pytest.skip(f"set {LIVE_FLAG}=1 and {WEBUI_E2E_FLAG}=1 to run Fileman WebUI live e2e")
    ...
    run_env["FILEMAN_ROLLBACK_HMAC_KEY"] = run_env.get("FILEMAN_ROLLBACK_HMAC_KEY") or "webui-live-e2e-key"
