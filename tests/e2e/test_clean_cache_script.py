...
def test_reset_workspace_state_recreates_movi_dir(tmp_path: Path):
    ...
    movi_runs = workspace / ".movi" / "runs"
    movi_runs.mkdir(parents=True)
    env = os.environ.copy()
    env["MOVI_WORKSPACE_ROOT"] = str(workspace)

    subprocess.run([str(script)], cwd=str(script_root), env=env, check=True)
    assert (workspace / ".movi").exists()
...
def test_prune_repo_runtime_does_not_touch_machine_cache_or_workspace(tmp_path: Path):
    ...
    (workspace / ".movi" / "runs").mkdir(parents=True)
    (machine_cache / "pycache" / "marker.pyc").write_text("x", encoding="utf-8")
    (workspace / ".movi" / "runs" / "marker.txt").write_text("x", encoding="utf-8")
    ...
    assert (workspace / ".movi" / "runs" / "marker.txt").exists()
...
def test_prune_machine_cache_dry_run_does_not_create_missing_targets(tmp_path: Path):
    ...
    env["MOVI_VENV_DIR"] = str(machine / "venv" / "default")
...
def test_prune_workspace_runtime_preserves_manifests_and_preferences(tmp_path: Path):
    ...
    workspace = tmp_path / "workspace"
    runs_root = workspace / ".movi" / "runs"
    artifact_root = workspace / ".movi" / "artifacts"
    manifest_root = workspace / ".movi" / "manifests"
    ...
    env = os.environ.copy()
    env["MOVI_RUN_BUNDLE_ROOT"] = str(runs_root)
    env["MOVI_ARTIFACT_ROOT"] = str(artifact_root)
    env["MOVI_MANIFEST_ROOT"] = str(manifest_root)
...
def test_prune_workspace_runtime_dry_run_does_not_create_missing_workspace_dirs(tmp_path: Path):
    ...
    workspace = tmp_path / "workspace"
    run_root = workspace / ".movi" / "runs"
    artifact_root = workspace / ".movi" / "artifacts"
    manifest_root = workspace / ".movi" / "manifests"

    env = os.environ.copy()
    env["MOVI_RUN_BUNDLE_ROOT"] = str(run_root)
    env["MOVI_ARTIFACT_ROOT"] = str(artifact_root)
    env["MOVI_MANIFEST_ROOT"] = str(manifest_root)
...
with log_path.open("a", encoding="utf-8") as handle:
    ...
    if args == ["system", "df", "-v"]:
        ...
        "movi-ci                        local    abc123         1 hour ago       3.32GB    0B            3.323GB       0\\n\\n"
        "Containers space usage:\\n\\n"
        "CONTAINER ID   IMAGE   COMMAND   LOCAL VOLUMES   SIZE   CREATED   STATUS   NAMES\\n\\n"
        "Local Volumes space usage:\\n\\n"
        "VOLUME NAME                              LINKS     SIZE\\n"
        "movi-web-stack_movi_playwright           0         972MB\\n"
        "movi-web-stack_movi_venv                 0         401.6MB\\n"
        "movi-web-stack_movi_webui_node_modules   0         0B\\n\\n"
        "Build cache usage: 188MB\\n"
        )
        raise SystemExit(0)
    if args == ["system", "df", "-v", "--format", "json"]:
        print(
            '{{"Images":[{{"Repository":"movi-ci","Tag":"local","UniqueSize":"3.323GB","Size":"3.32GB"}}],'
            '"Containers":[],'
            '"Volumes":['
            '{{"Name":"movi-web-stack_movi_playwright","Size":"972MB","Mountpoint":"/docker/playwright"}},'
            '{{"Name":"movi-web-stack_movi_venv","Size":"401.6MB","Mountpoint":"/docker/venv"}},'
            '{{"Name":"movi-web-stack_movi_webui_node_modules","Size":"0B","Mountpoint":"/docker/node_modules"}}'
            '],'
            '"BuildCache":['
        ...
        raise SystemExit(0)
if args[:3] == ["image", "inspect", "movi-ci:local"]:
    print("884283278")
    raise SystemExit(0)
if args[:2] == ["volume", "inspect"]:
    name = args[2]
    print(f'{{"com.docker.compose.project":"movi-web-stack"}} /tmp/{{name}}')
    raise SystemExit(0)
...
""",
    ...
    assert "image rm -f movi-ci:local" in calls
    assert "volume rm -f movi-web-stack_movi_playwright" in calls
    assert "volume rm -f movi-web-stack_movi_venv" in calls
    assert "volume rm -f movi-web-stack_movi_webui_node_modules" in calls
...
def test_prune_shared_runner_workdirs_respects_worker_guard_and_keeps_runner_layers(tmp_path: Path):
    ...
    runner_root = tmp_path / "movi-shared-runners"
