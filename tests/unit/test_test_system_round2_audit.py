...
def test_mutation_manual_workflow_uses_hash_locked_mutmut_install() -> None:
    ...
    _assert_contains(workflow_text, 'echo "FILEMAN_CI_IMAGE=$IMAGE_REF" >> "$GITHUB_ENV"', "manual image env export")
    _assert_contains(
        workflow_text,
...
def test_quality_gate_runs_fast_lane_before_full_pytest() -> None:
    ...
    full_lane_pattern = (
        ...
        r"\s+\\\s*env -u PRE_COMMIT_FROM_REF -u PRE_COMMIT_TO_REF FILEMAN_RUN_LIVE_TESTS=0"
    )
...
def test_pre_commit_ai_context_and_ci_change_detection_are_config_driven() -> None:
    ...
    assert 'step fast-lane env FILEMAN_ALLOW_HOST_EXECUTION=1 bash "$ROOT/scripts/local_quality_gate.sh" fast' not in pre_push_gate
...
def test_prepush_gate_defaults_to_standard_mode() -> None:
    ...
    _assert_contains(pre_push_gate, 'MODE="${1:-${FILEMAN_PRE_PUSH_MODE:-standard}}"', "pre-push default mode standard")
...
def test_ci_change_detection_expands_heavy_scope_and_catches_deletions() -> None:
    ...
    assert "apps/cli/fileman.py" in heavy_globs, "change-detection core CLI entrypoint"
...
def test_local_ci_matrix_gate_enforces_strict_pytest_markers_and_config() -> None:
    ...
    _assert_contains(
        matrix_gate,
        "directly from /opt/fileman-ci-venv",
        "local matrix uses the image-baked runtime venv directly",
    )
    _assert_contains(
        matrix_gate,
        'export FILEMAN_VENV_DIR="/opt/fileman-ci-venv"',
...
def test_live_integration_requires_secrets_and_uses_image_contract() -> None:
    ...
    _assert_contains(live_ci, "missing required secret FILEMAN_LIVE_TEST_URL", "live integration url preflight")
...
def test_env_example_points_to_workspace_runtime_env() -> None:
    ...
    _assert_contains(env_contract_doc, "<workspace-root>/.fileman/env/runtime.env", "env contract workspace runtime env path")
    _assert_contains(env_example, "<workspace-root>/.fileman/env/runtime.env", "env example workspace runtime env path")
...
def test_env_contract_live_coverage_vars_and_budgets_are_consistent() -> None:
    ...
    assert "FILEMAN_LIVE_COVERAGE_FILE" in contract
    assert len(contract) <= 59

    live_count = sum(1 for name in contract if name.startswith("LIVE_"))
    fileman_count = sum(1 for name in contract if name.startswith("FILEMAN_"))
    assert live_count <= budgets["LIVE_"]
    assert fileman_count <= budgets["FILEMAN_"]
