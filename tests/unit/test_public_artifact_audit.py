...
def test_public_artifact_audit_fails_when_audited_file_is_undeclared(tmp_path: Path) -> None:
    ...
    _write_policy(repo)
    (repo / "apps" / "webui" / "public" / "fileman-mark.svg").write_text("<svg/>", encoding="utf-8")
    ...
    (repo / "contracts" / "governance" / "public_asset_provenance.yaml").write_text(
        "\n".join(
            [
                "version: 1",
                "assets:",
                "  - path: apps/webui/public/fileman-mark.svg",
...
def test_public_artifact_audit_fails_on_forbidden_extension_or_secret_like_content(tmp_path: Path) -> None:
    ...
    (repo / "docs").mkdir(parents=True)
    _write_policy(repo)
    (repo / "apps" / "webui" / "public" / "fileman-mark.svg").write_text("Authorization: Bearer token\n", encoding="utf-8")
