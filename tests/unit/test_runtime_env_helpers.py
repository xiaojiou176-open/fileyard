...
def test_workspace_root_runtime_env_and_resolve_helpers(monkeypatch, tmp_path: Path) -> None:
    ...
    runtime_file = workspace / ".fileorganize" / "env" / "runtime.env"
    ...
    runtime_file.write_text(
        "\n".join(
            [
                ...
                'FILEORGANIZE_INPUT_ROOT="/tmp/input root"',
                "IGNORED_LINE",
            ]
        )
        + "\n",
    ...
    monkeypatch.setenv("FILEORGANIZE_WORKSPACE_ROOT", str(workspace))
    monkeypatch.setenv("GEMINI_API_KEY", "from_env")
    monkeypatch.delenv("FILEORGANIZE_INPUT_ROOT", raising=False)
    ...
    assert runtime_env.resolve_path_value("FILEORGANIZE_INPUT_ROOT", "/tmp/default", root=workspace) == Path("/tmp/input root")


def test_upsert_runtime_env_merges_and_drops_blank_values(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    env_path = workspace / ".fileorganize" / "env" / "runtime.env"
