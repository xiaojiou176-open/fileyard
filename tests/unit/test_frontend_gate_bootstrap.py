...
def test_lint_frontend_keeps_canonical_container_path_even_during_host_emergency_gate() -> None:
    ...
    assert ('env -u MOVI_ALLOW_HOST_EXECUTION MOVI_COMPOSE_SERVICE=movi-web-api bash "$ROOT/scripts/container_exec.sh"') in script
...
def test_run_webui_task_allows_explicit_host_execution_outside_ci() -> None:
    ...
    assert "MOVI_ALLOW_HOST_EXECUTION=1 is forbidden in CI" in script
...
def test_env_contract_registers_web_stack_runtime_vars() -> None:
    ...
    assert "MOVI_WEBUI_HOST" in registry
    assert "MOVI_WEBUI_PORT" in registry
    assert "MOVI_WEB_API_HOST" in registry
    assert "MOVI_WEB_API_PORT" in registry
...
def test_env_contract_registers_live_coverage_runtime_vars() -> None:
    ...
    assert "MOVI_LIVE_COVERAGE_FILE" in registry
    assert "LIVE_COVERAGE_FILE" in registry
    assert "MOVI_LIVE_COVERAGE_FILE=" in env_example
