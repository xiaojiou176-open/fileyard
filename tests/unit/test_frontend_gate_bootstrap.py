...
def test_lint_frontend_keeps_canonical_container_path_even_during_host_emergency_gate() -> None:
    ...
    assert ('env -u FILEMAN_ALLOW_HOST_EXECUTION FILEMAN_COMPOSE_SERVICE=fileman-web-api bash "$ROOT/scripts/container_exec.sh"') in script
...
def test_run_webui_task_allows_explicit_host_execution_outside_ci() -> None:
    ...
    assert "FILEMAN_ALLOW_HOST_EXECUTION=1 is forbidden in CI" in script
...
def test_env_contract_registers_web_stack_runtime_vars() -> None:
    ...
    assert "FILEMAN_WEBUI_HOST" in registry
    assert "FILEMAN_WEBUI_PORT" in registry
    assert "FILEMAN_WEB_API_HOST" in registry
    assert "FILEMAN_WEB_API_PORT" in registry
...
def test_env_contract_registers_live_coverage_runtime_vars() -> None:
    ...
    assert "FILEMAN_LIVE_COVERAGE_FILE" in registry
    assert "LIVE_COVERAGE_FILE" in registry
    assert "FILEMAN_LIVE_COVERAGE_FILE=" in env_example
