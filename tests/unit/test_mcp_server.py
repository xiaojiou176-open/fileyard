...
def _isolated_runtime_env(tmp_path: Path) -> dict[str, str]:
    ...
    manifest_root = workspace_root / ".fileman" / "manifests"
    artifact_root = workspace_root / ".fileman" / "artifacts"
    run_root = workspace_root / ".fileman" / "runs"
    for path in (input_root, output_root, manifest_root, artifact_root, run_root):
        path.mkdir(parents=True, exist_ok=True)
    return {
        **os.environ,
        "PYTHONPATH": str(Path.cwd()),
        "FILEMAN_WORKSPACE_ROOT": str(workspace_root),
        "FILEMAN_INPUT_ROOT": str(input_root),
        "FILEMAN_OUTPUT_ROOT": str(output_root),
        "FILEMAN_MANIFEST_ROOT": str(manifest_root),
        "FILEMAN_ARTIFACT_ROOT": str(artifact_root),
        "FILEMAN_RUN_BUNDLE_ROOT": str(run_root),
    }


def test_mcp_catalog_exposes_safe_v1_tools_and_resources(tmp_path: Path) -> None:
    env = _isolated_runtime_env(tmp_path)
    previous_env = {key: os.environ.get(key) for key in env if key.startswith("FILEMAN_")}
    try:
        for key, value in env.items():
            if key.startswith("FILEMAN_"):
    ...
    assert "fileman://workflow/safety-boundary" in resource_uris
    assert "fileman://workflow/tool-matrix" in resource_uris
    ...
    async def run() -> None:
        ...
        async with stdio_client(server) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                assert result.serverInfo.name == "Fileman MCP"
                ...
                safety = await session.read_resource("fileman://workflow/safety-boundary")
                assert safety.contents
                assert "review-first" in safety.contents[0].text

                tool_matrix = await session.read_resource("fileman://workflow/tool-matrix")
                ...
                assert "fileman://docs/{doc_id}" in template_uris

                docs_resource = await session.read_resource("fileman://docs/mcp")
                assert docs_resource.contents
                assert "Fileman MCP v1" in docs_resource.contents[0].text
