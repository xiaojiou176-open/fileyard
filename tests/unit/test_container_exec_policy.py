...
def _running_inside_container() -> bool:
    return Path("/.dockerenv").exists() or os.getenv("MOVI_IN_CONTAINER") == "1"
...
def _run_container_exec_with_fake_docker(
    ...
    *,
    ...
    env = os.environ.copy()
    env.pop("MOVI_IN_CONTAINER", None)
    env.pop("MOVI_ALLOW_HOST_EXECUTION", None)
    env.pop("MOVI_CI_IMAGE", None)
...
def test_container_exec_rejects_spoofed_in_container_env_on_host() -> None:
    ...
    env = os.environ.copy()
    env.pop("CI", None)
    env["MOVI_IN_CONTAINER"] = "1"
    env["MOVI_ALLOW_HOST_EXECUTION"] = "0"
...
def test_container_exec_image_path_mounts_isolated_runtime_and_keeps_ci_env_passthrough(
    ...
    env.pop("MOVI_IN_CONTAINER", None)
    env.pop("MOVI_ALLOW_HOST_EXECUTION", None)
    env["MOVI_CI_IMAGE"] = "movi-ci:test"
...
""",
    ...
    assert any(
        call.startswith(
            ...
            f"--tag movi-ci:local {repo_root}"
        )
        for call in calls
    )
    assert any("compose" in call and "run --name" in call and "--rm -T" in call and "movi-ci" in call for call in calls)
...
""",
    ...
    assert proc.returncode == 1
    assert "failed to build local fallback image movi-ci:local" in proc.stderr
    assert "Likely cause: DNS/network failure while building the local CI image." in proc.stderr
    assert "This gate can only run offline if movi-ci:local already exists locally." in proc.stderr
...
if [ "$#" -ge 1 ] && [ "$1" = "build" ]; then
  touch "{marker}"
  echo 'ERROR: failed to solve: image "docker.io/library/movi-ci:local": already exists' >&2
  exit 1
fi
if [ "$#" -ge 1 ] && [ "$1" = "compose" ]; then
  exit 0
