...
def _prepare_env(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("FILEMAN_ROLLBACK_HMAC_KEY", "unit-test-web-api-hmac-key")
...
def test_runtime_settings_roundtrip(monkeypatch, tmp_path: Path):
    ...
    assert f"FILEMAN_INPUT_ROOT={new_input.resolve()}" in rendered
    assert f"FILEMAN_OUTPUT_ROOT={new_output.resolve()}" in rendered
...
def test_rollback_strict_integrity_missing_key_fails_before_enqueue(monkeypatch, tmp_path: Path):
    ...
    assert rollback_resp.json()["detail"] == "strict_integrity=true requires FILEMAN_ROLLBACK_HMAC_KEY"
...
def test_web_api_validation_errors_and_static_hosting(monkeypatch, tmp_path: Path):
    ...
    invalid_mode = client.post(
        "/api/jobs/analyze",
        data={"input_mode": "weird", "input_directory": "~/.fileman/workspaces/default/data/raw"},
    )
