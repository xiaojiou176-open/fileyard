...
def test_parse_args_uses_env_defaults(monkeypatch) -> None:
    monkeypatch.setenv("FILEYARD_WEB_API_HOST", "0.0.0.0")
    monkeypatch.setenv("FILEYARD_WEB_API_PORT", "19090")
...
def test_parse_args_prefers_cli_args_over_env(monkeypatch) -> None:
    monkeypatch.setenv("FILEYARD_WEB_API_HOST", "127.0.0.5")
    monkeypatch.setenv("FILEYARD_WEB_API_PORT", "19555")
...
def test_module_main_branch_uses_env_defaults(monkeypatch) -> None:
    ...
    monkeypatch.setitem(sys.modules, "uvicorn", types.SimpleNamespace(run=fake_run))
    monkeypatch.setenv("FILEYARD_WEB_API_HOST", "127.0.0.3")
    monkeypatch.setenv("FILEYARD_WEB_API_PORT", "18123")
    monkeypatch.setattr(sys, "argv", ["fileyard-web-api"])
