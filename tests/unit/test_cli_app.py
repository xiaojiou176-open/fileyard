...
def test_read_runtime_env_value_and_resolve_env(tmp_path, monkeypatch):
    ...
    runtime_env = workspace_root / ".fileman" / "env" / "runtime.env"
...
def test_default_report_out_fallback_when_script_dir_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FILEMAN_WORKSPACE_ROOT", str(tmp_path / "workspace"))
    assert cli_app._default_report_out() == str(tmp_path / "workspace" / ".fileman" / "artifacts" / "report" / "report_summary.json")
